...
class AudioComponent(util.Component):
    ...
    async def set_master_volume(
        self, volume: RobotVolumeLevel
    ) -> protocol.MasterVolumeResponse:
    ...
    def _open_file(self, filename):
        ...
        if (
            _params.sampwidth != 2
            or _params.nchannels != 1
            or _params.framerate > 16025
            or _params.framerate < 8000
        ):
    ...
    async def _request_handler(self, reader, params, volume):
        ...
        msg = protocol.ExternalAudioStreamPrepare(
            audio_frame_rate=params.framerate, audio_volume=volume
        )
        ...
        while frames > 0 and not self._done_event.is_set():
            ...
            msg = protocol.ExternalAudioStreamChunk(
                audio_chunk_size_bytes=len(audio_data), audio_chunk_samples=audio_data
            )
            ...
            time_ahead = (
                curr_chunk * DEFAULT_FRAME_SIZE - expected_data_count
            ) / params.framerate
            if time_ahead > 1.0:
                self.logger.debug(
                    "waiting %f to catchup chunk %f", time_ahead - 0.5, curr_chunk
                )
