...
def get_serial(serial=None):
    if not serial:
        ...
        if not serial:
            ...
            print(
                "Found robot serial number in environment variable '{}'".format(
                    colored("ANKI_ROBOT_SERIAL", "green")
                )
            )
    serial = serial.lower()
...
def user_authentication(session_id: bytes, cert: bytes, ip: str, name: str) -> str:
    ...
    print(
        "Attempting to download guid from {} at {}:443...".format(
            colored(name, "cyan"), colored(ip, "cyan")
        ),
        end="",
    )
    sys.stdout.flush()
...
def get_name_and_ip(robot_name=None, ip=None):
    if not robot_name:
        ...
        if not robot_name:
            ...
            print(
                "Found robot name in environment variable '{}'".format(
                    colored("VECTOR_ROBOT_NAME", "green")
                )
            )
    ...
    if not ip:
        ...
        if not ip:
            print(
                ...
                "then raising and lowering his arms. If you see {} on his face, reconnect Vector to your WiFi using the Vector Companion App.".format(
                    colored("XX.XX.XX.XX", "red")
                )
            )
            ip = input("Enter robot ip: ")
        else:
            print(
                "Found robot ip address in environment variable '{}'".format(
                    colored("ANKI_ROBOT_HOST", "green")
                )
            )
        ...
        for fields in cert.subject:
            ...
            if "commonName" in current:
                ...
                if common_name != robot_name:
                    print(colored(" ERROR", "red"))
                    sys.exit(
                        "The name of the certificate ({}) does not match the name provided ({}).\n"
                        "Please verify the name, and try again.".format(
                            common_name, robot_name
                        )
                    )
                else:
                    return
...
def main():
    ...
    parser.add_argument(
        ...
        "--ip",
        help=(
            "Your robot ip address (ex. 192.168.42.42). "
            ...
            "reconnect Vector to your WiFi using the Vector Companion App.".format(
                colored("XX.XX.XX.XX", "red")
            )
        ),
    )
    parser.add_argument(
        ...
        "--name",
        help=(
            "Your robot name (ex. Vector-A1B2). "
    ...
    parser.add_argument(
        ...
        "--serial",
        help=(
            "Your robot serial number (ex. 00e20100). "
    ...
    parser.add_argument(
        ...
        dest="new_ip",
        help=(
            "Update the stored ip for Vector. This makes it easier to transfer between networks."
        ),
    )
