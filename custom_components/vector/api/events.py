...
class Events(Enum):
    ...
    robot_observed_motion = (
        "robot_observed_motion"  #: Robot event dispatched when Vector observes motion.
    )
    robot_observed_face = (
...
class EventHandler:
    ...
    def start(self, connection: Connection):
        ...
        self._thread = threading.Thread(
            target=self._run_thread, daemon=True, name="Event Stream Handler Thread"
        )
        self._thread.start()
    ...
    def _run_thread(self):
        try:
            ...
            self.event_future = asyncio.run_coroutine_threadsafe(
                self._handle_event_stream(), self._conn.loop
            )
    ...
    def _done_callback(self, completed_future):
        ...
        if exc:
            ...
            if isinstance(exc, TypeError) and "positional arguments but" in str(exc):
                self.logger.error(
                    "The subscribed function may be missing parameters in its definition. Make sure it has robot, event_type and event positional parameters."
                )
        ...
        try:
            ...
            async for evt in self._conn.grpc_interface.EventStream(req):
                ...
                try:
                    ...
                    await self.dispatch_event_by_name(
    ...
    def unsubscribe_by_name(self, func: Callable, event_name: str):
        ...
        if event_name in self.subscribers.keys():
            ...
            if func in event_subscribers:
                ...
                self.logger.error(
                    f"The function '{func.__name__}' is not subscribed to '{event_name}'"
                )
        else:
            self.logger.error(
                f"Cannot unsubscribe from event_type '{event_name}'. "
